...
#define RulerLong       kFitH(40)
#define RulerShort      kFitH(30)
#define TrangleWidth    kFitH(16)
#define CollectionHeight kFitH(80)
...
-(void)drawRect:(CGRect)rect{
    ...
    for (int i = 0; i <= _betweenNumber; i ++){
        ...
        if (i%_betweenNumber == 0){
            ...
            NSDictionary *attribute = @{NSFontAttributeName:TextRulerFont,NSForegroundColorAttributeName:COLOR_WITH_HEX(0x9F9F9F)};
            ...
            [num drawInRect:CGRectMake(startX+lineCenterX*i-width/2, longLineY+kFitH(5), width, 14) withAttributes:attribute];
...
-(void)drawRect:(CGRect)rect{
    ...
    NSDictionary *attribute = @{NSFontAttributeName:TextRulerFont,NSForegroundColorAttributeName:COLOR_WITH_HEX(0x9F9F9F)};
    CGFloat width = [num boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:0 attributes:attribute context:nil].size.width;
    [num drawInRect:CGRectMake(rect.size.width-width/2, longLineY+kFitH(5), width, 14) withAttributes:attribute];
...
-(void)drawRect:(CGRect)rect{
    ...
    NSDictionary *attribute = @{NSFontAttributeName:TextRulerFont,NSForegroundColorAttributeName:COLOR_WITH_HEX(0x9F9F9F)};
    CGFloat width = [num boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:0 attributes:attribute context:nil].size.width;
    [num drawInRect:CGRectMake(0-width/2, longLineY+kFitH(5), width, 14) withAttributes:attribute];
...
-(UITextField *)valueTF{
    if (!_valueTF) {
        _valueTF  = [[UITextField alloc]initWithFrame:CGRectMake((kScreenWidth - 150)/2, 0,150, kFitW(30))];
        _valueTF.userInteractionEnabled   = NO;
        _valueTF.textAlignment = NSTextAlignmentCenter;
        _valueTF.defaultTextAttributes    = @{
                                              NSFontAttributeName:kBoldFont(19),
...
-(UILabel *)unitLab{
//    if (!_unitLab) {
//        _unitLab = [[UILabel alloc]initWithFrame:CGRectMake( CGRectGetMaxX(_valueTF.frame) , 0, kFitW(30), kFitW(40))];
//        _unitLab.textColor = kMainColor;
//    }
    return _unitLab;
}
...
-(void)setDefaultValue:(float)defaultValue animated:(BOOL)animated{
    ...
    _valueTF.text   = [NSString stringWithFormat:@"%.1f%@",defaultValue,_unit];
...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if ([newStr intValue]>_maxValue){
        _valueTF.text =  [NSString stringWithFormat:@"%.f%@",_maxValue,_unit];
...
-(void)didChangeValue{
    ...
    if ((textFieldValue-_minValue)>=0) {
        ...
        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:kLocalized(@"提示") message:kLocalized(@"少于最低值，请重新输入") delegate:self cancelButtonTitle:kLocalized(@"确定") otherButtonTitles:nil, nil];
        [alertView show];
    }
}
...
//        }
        rulerView.backgroundColor = self.bgColor;
